...
while (1 == 1) {
    ...
    open (LOCK, ">/kunden/homepages/17/d105428537/htdocs/batchsubmit/lock.xml") || die $!;

    if (flock(LOCK,2)) {

	open(TASKREAD, "/kunden/homepages/17/d105428537/htdocs/batchsubmit/task.xml") || die $!;
	...
	$basedir = "/kunden/homepages/17/d105428537/htdocs/batchsubmit";
...
#						$cmndw = "\"$rpath\\$rexe\" $rcmd $rbatch $rsave $rrestore \"$basedir$program\" \"$basedir\\$wpath\\$printfile\"";		
						...
						copy("$basedir\/placeholder.xml","$basedir\/$upath\/$printfile") || die $!;						
					}				
...
#						$cmndw = "\"$spath\\$sasexe\" $noautoexec -noxwait -initstmt '%include \"parm.inc\";' $sysin $basedir$tprogram $log $basedir\\$wpath\\$logfile $print $basedir\\$wpath\\$printfile";		
						...
						copy("$basedir\/placeholder.xml","$basedir\/$upath\/$logfile") || die $!;		
						copy("$basedir\/placeholder.xml","$basedir\/$upath\/$printfile") || die $!;	
